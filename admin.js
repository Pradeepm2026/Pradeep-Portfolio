...
addMomentPhotoButton.addEventListener('click', () => { const file = momentImageInput.files[0]; if (!file) return; if (pendingMomentFiles.length >= 10) { momentStatus.textContent = 'Maximum 10 photos allowed.'; return; } pendingMomentFiles.push(file); momentImageInput.value = ''; momentPhotoCount.textContent = `${pendingMomentFiles.length} of 10 photos added.`; });
momentUploadForm.addEventListener('submit', async (event) => { event.preventDefault(); if (!pendingMomentFiles.length) { momentStatus.textContent = 'Add at least one photo first.'; return; } momentStatus.textContent = 'Uploading moments...'; try { const formData = new FormData(); formData.append('description', document.querySelector('#moment-description').value.trim()); pendingMomentFiles.forEach((file) => formData.append('momentImage', file)); const response = await fetch('/api/moments', { method: 'POST', headers: getAdminHeaders(), body: formData }); const result = await response.json(); if (!response.ok) throw new Error(result.message); momentStatus.textContent = result.message; momentUploadForm.reset(); pendingMomentFiles = []; momentPhotoCount.textContent = '0 of 10 photos added.'; showAdminToast('Moment photos uploaded successfully.'); loadAdminMoments(result.moment._id); } catch (error) { momentStatus.textContent = error.message || 'Moment upload failed.'; } });
